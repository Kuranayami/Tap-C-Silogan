...
import { Mail, Phone, ArrowLeft, Loader2, Check } from 'lucide-react'
...
export default function LoginPage({ onLogin, onBack }) {
  ...
  const handleGoogleLogin = () => {
    const name = googleName.trim() || 'Customer'
    const mockUser = { id: 'google-' + Date.now(), name, email: '', auth_provider: 'google' }
          ...
          <AnimatePresence mode="wait">
            {mode === 'otp' ? (
              ...
              <motion.div key="select" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                ...
                {/* Google with name input */}
                <div className="space-y-2">
                  <div className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#27272a] bg-[#09090b] focus-within:border-[#f97316]/50 transition-colors">
                    ...
                    <button
                      ...
                      className="shrink-0 p-1 rounded-lg bg-[#f97316] hover:bg-[#ea580c] text-white transition-all disabled:opacity-50"
